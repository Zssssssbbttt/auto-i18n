...
2. 运行 `node scripts/i18n-scan/index.cjs -i`，会自动生成 `src/locales/th.json` 骨架文件，并在 `index.ts` 中注册该语言

3. 手动翻译 `th.json` 中的空字符串，或运行 `-t` 使用 AI 翻译

### 移除语言

1. 在 `i18n.config.js` 的 `targetLanguages` 中删除语言代码：
   ```js
   targetLanguages: ['en']  // 移除了泰语
   ```

2. 运行 `node scripts/i18n-scan/index.cjs -i`，会自动从 `index.ts` 中移除该语言的注册（import、messages 条目等）

3. **注意**：脚本不会删除已有的语言包文件（如 `th.json`），防止误删。如需彻底清理，请手动删除该文件。
